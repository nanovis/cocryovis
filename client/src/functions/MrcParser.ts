...
export async function convertMRCToRaw(
    mrcFile: File
  ): Promise<{ rawFile: File; jsonFile: File }> {
        ...
        while (true) {
          ...
          if (completeLength > 0) {
            ...
            if (range === 0) {
              ...
              for (let i = 0; i < floatChunk.length; i++) {
                outChunk[i] = Math.round(((floatChunk[i] - minVal) / range) * 255);
              }
            }
    ...
    // Generate the JSON settings object.
    const settings = {
      file: rawFileName,
      size: { x: nx, y: ny, z: nz },
    ...
    // Create a JSON File from the settings.
    const jsonFileName = mrcFile.name.replace(/\.mrc$/i, ".json");
